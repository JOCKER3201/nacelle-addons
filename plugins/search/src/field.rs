//! The query box: the toolkit's field, drawn through the plugin ABI.
//!
//! # Why this file exists at all
//!
//! The model half of `object::text_input` is used verbatim — every key,
//! every caret step, every undo group and the whole IME contract come
//! from there ([`crate::finder`]). The VIEW half cannot be: it draws
//! through `Ctx`, which carries the host's draw list, its font system
//! and its focus chain, and none of those three cross a dynamic library
//! boundary. A plugin has a function table and an opaque handle.
//!
//! So this is that view again, over [`Surface`] instead of `Ctx`, from
//! the SAME tokens — `[field]` for the geometry, `component.field.*` for
//! the colours, class `field` for the state wash, `motion.caret_blink.*`
//! for the blink. Not one number here is this file's own: a token the
//! master does not declare answers zero and the part it decides simply
//! is not drawn, which is the degradation every widget in this tree
//! shares.
//!
...
//! focus chain for a plugin to hold a `Ctx` — this file is deleted and
//! the call goes to the toolkit's own. Until then two copies of a FIELD
//! are better than a plugin that cannot be typed into.
//!
//! What is deliberately NOT here: the mask. A search box is never a
