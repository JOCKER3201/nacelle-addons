...
//!
//! A copy of `plugins/search/src/field.rs`, which is itself the named
//! copy of the toolkit's field VIEW — the model half of
//! `object::text_input` is used verbatim ([`crate::model`]), and the
//! view half cannot be, because it draws through `Ctx` and a plugin has
//! a function table and an opaque handle. Read the search copy's header
//! for that argument in full; nothing about it is different here.
//!
//! What IS different is only what the copy is for: the search panel
//! types a query, this panel types a path. Same tokens (`[field]`,
//! `component.field.*`, class `field`, `motion.caret_blink.*`), same
//! degradation — a token the master does not declare answers zero and
//! the part it decides simply is not drawn.
//!
//! It is duplication twice over now, and worth naming as such: the day
//! the object layer grows a `Surface`-based field — or the ABI grows
//! enough of a focus chain for a plugin to hold a `Ctx` — BOTH copies
//! are deleted and the call goes to the toolkit's own. Until then two
//! panels that can be typed into beat one shared file this repository
//! has no crate to put in.
//!
...
pub fn draw(
    ...
    // is cut and `field.corner_style` carries HOW. Both readers are the
    // toolkit's, so this box and the object layer's field cut their
    // corners by one rule — see the search copy for the regression that
    // taught it.
