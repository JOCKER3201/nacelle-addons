//! The prompt box: the toolkit's field, drawn through the plugin ABI.
//!
...
//!
//! It is duplication over again — search and the AI tool panels carry
//! the same file — and it is worth naming as such: the day the object
//! layer grows a `Surface`-based field, EVERY copy is deleted and the
//! call goes to the toolkit's own. Until then a copy of a FIELD is
//! better than a plugin that cannot be typed into.
//!
//! What is deliberately NOT here: the mask. A prompt box is never a
