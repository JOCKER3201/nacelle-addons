...
fn button(sf: &mut impl Surface, r: Rect, caption: &str, state: State) {
    ...
    // own bed. Without it a plugin button is a near-transparent tint that
    // takes the colour of whatever is behind it — the reason buttons did
    // not match window to window (JEDEN MODEL OKNA).
